...
// TODO: does it work at all? when run `astro dev` it doesn't error if the DB url is missing?!
...
export const serverEnv = createEnv({
  ...
  server: {
    ...
    // propel auth
    PROPELAUTH_API_KEY: z.string().min(1).optional(),
    PROPELAUTH_VERIFIER_KEY: z.string().min(1).optional(),
    // optional Fogbender
  ...
  client: {
    // propel auth
    PUBLIC_AUTH_URL: z.string().min(1).optional(),
    // fogbender
...
- To fix it, go to http://localhost:4321/setup and follow the instructions there
